...
/reagent/nutrition/on_metabolize_plant(var/obj/structure/interactive/plant/plant,var/reagent_container/container,var/starting_volume=0,var/multiplier=1)
	...
	if(nutrition_amount)
		plant.add_nutrition(.*nutrition_amount/14) //plant food has 14 nutrition, so we want 1u plant food = 1 nutrition point. //*(1/14)*(1/60)*100

	if(hydration_amount)
		plant.add_hydration(.*hydration_amount/25) //water has 25 hydration, and we want 1u water = 1 hydration point  //*(1/25)*(1/80)*100
