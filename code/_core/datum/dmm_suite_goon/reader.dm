...
dmm_suite
	...
	*/
	read_map(dmm_text as text, coordX as num, coordY as num, coordZ as num, tag as text)
		...
		loadModel(atomPath, list/attributes, list/strings, xcrd, ycrd, zcrd)
			...
			// Handle Turfs & Movable Atoms
				...
				if(ispath(atomPath, /turf))
					...
					//instance = location.ReplaceWith(atomPath, handle_air = 0, handle_dir = 0)
					//instance = location.change_turf(atomPath)
				else
					instance = new atomPath(location)
