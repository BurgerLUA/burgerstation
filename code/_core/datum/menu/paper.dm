	...
	if(href_list && usr && is_player(usr))
		...
		if(href_list["edit"])
			...
			if(desired_new_text && length(desired_new_text))
				...
				if(desired_new_text && length(desired_new_text))
					//desired_new_text = desired_new_text
					P.active_paper.data[desired_page] = desired_new_text
					set_text(P,sanitize(P.active_paper.data[desired_page],10000,extra = FALSE),P.active_paper.name,desired_page,length(P.active_paper.data))
