/loot/weapon/rifle/abakan
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/an94,
		/obj/item/weapon/ranged/bullet/magazine/rifle/an94,
		/obj/item/magazine/rifle_545,
		/obj/item/magazine/rifle_545,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/an94 = 1,
		/obj/item/magazine/rifle_545 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/advanced
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/advanced,
		/obj/item/weapon/ranged/bullet/magazine/rifle/advanced,
		/obj/item/magazine/sniper_762,
		/obj/item/magazine/sniper_762,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/advanced = 1,
		/obj/item/magazine/sniper_762 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/ak12
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/ak12,
		/obj/item/weapon/ranged/bullet/magazine/rifle/ak12,
		/obj/item/magazine/rifle_545,
		/obj/item/magazine/rifle_545,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/ak12 = 1,
		/obj/item/magazine/rifle_545 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/ak47
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/ak47,
		/obj/item/weapon/ranged/bullet/magazine/rifle/ak47,
		/obj/item/magazine/rifle_762_short,
		/obj/item/magazine/rifle_762_short,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/ak47 = 1,
		/obj/item/magazine/rifle_762_short = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/assault
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/assault,
		/obj/item/weapon/ranged/bullet/magazine/rifle/assault,
		/obj/item/magazine/rifle_308,
		/obj/item/magazine/rifle_308
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/assault = 1,
		/obj/item/magazine/rifle_308 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/burst
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/burst,
		/obj/item/weapon/ranged/bullet/magazine/rifle/burst,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/burst = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/carbine
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/carbine,
		/obj/item/weapon/ranged/bullet/magazine/rifle/carbine,
		/obj/item/magazine/carbine_223,
		/obj/item/magazine/carbine_223,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/carbine = 1,
		/obj/item/magazine/carbine_223 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/civ_carbine
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/civ_carbine,
		/obj/item/weapon/ranged/bullet/magazine/rifle/civ_carbine,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/civ_carbine = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/dmca
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/dmca,
		/obj/item/weapon/ranged/bullet/magazine/rifle/dmca,
		/obj/item/magazine/minigun_46,
		/obj/item/magazine/minigun_46,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/dmca = 1,
		/obj/item/magazine/minigun_46 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/gauss_gun
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/gauss_gun
	)

	loot_table = list(
		/obj/item/magazine/gauss_gun = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/gauss_carbine
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/gauss_gun/auto
	)

	loot_table = list(
		/obj/item/magazine/gauss_gun = 1
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/groza
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/groza,
		/obj/item/weapon/ranged/bullet/magazine/rifle/groza,
		/obj/item/magazine/rifle_939,
		/obj/item/magazine/rifle_939,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/groza = 1,
		/obj/item/magazine/rifle_939 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/heavy_sniper
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/heavy_sniper,
	)

	loot_table = list(
		/obj/item/magazine/sniper_50 = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/lmg
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/lmg,
	)

	loot_table = list(
		/obj/item/magazine/lmg_223 = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/lmg_nt
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/lmg_nt,
	)

	loot_table = list(
		/obj/item/magazine/lmg_762 = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/lmg_nt_light
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/lmg_nt_light,
	)

	loot_table = list(
		/obj/item/magazine/lmg_556 = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/marksman
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/marksman,
		/obj/item/weapon/ranged/bullet/magazine/rifle/marksman,
		/obj/item/magazine/rifle_308,
		/obj/item/magazine/rifle_308,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/marksman = 1,
		/obj/item/magazine/rifle_308 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/minigun //Lavaland only.
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/minigun,
	)

	loot_table = list(
		/obj/item/magazine/minigun_46 = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/nt_carbine
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/nt_carbine,
		/obj/item/weapon/ranged/bullet/magazine/rifle/nt_carbine,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/nt_carbine = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/pdw
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/pdw,
		/obj/item/weapon/ranged/bullet/magazine/rifle/pdw,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/pdw = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/pulse
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/pulse,
		/obj/item/weapon/ranged/bullet/magazine/rifle/pulse,
		/obj/item/magazine/pulse_rifle,
		/obj/item/magazine/pulse_rifle,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/pulse = 1,
		/obj/item/magazine/pulse_rifle = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/rpk
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/pkm,
	)

	loot_table = list(
		/obj/item/magazine/lmg_762_r = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/semi
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/semi,
		/obj/item/weapon/ranged/bullet/magazine/rifle/semi,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/semi = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/service
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/service,
		/obj/item/weapon/ranged/bullet/magazine/rifle/service,
		/obj/item/magazine/sniper_762,
		/obj/item/magazine/sniper_762,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/service = 1,
		/obj/item/magazine/sniper_762 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/sniper_127
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/sniper_127,
	)

	loot_table = list(
		/obj/item/magazine/sniper_127 = 1
	)
	loot_count = 6
	chance_none = 50

/loot/weapon/rifle/standard
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/standard,
		/obj/item/weapon/ranged/bullet/magazine/rifle/standard,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/standard = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/svt
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/svt,
		/obj/item/weapon/ranged/bullet/magazine/rifle/svt,
		/obj/item/magazine/rifle_762_long,
		/obj/item/magazine/rifle_762_long,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/svt = 1,
		/obj/item/magazine/rifle_762_long = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/syringe
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/syringe,
		/obj/item/magazine/syringe_gun,
		/obj/item/magazine/syringe_gun,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/syringe = 1,
		/obj/item/magazine/syringe_gun = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/tungsten
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/tungsten,
		/obj/item/magazine/rifle_tungsten,
		/obj/item/magazine/rifle_tungsten,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/tungsten = 1,
		/obj/item/magazine/rifle_tungsten = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/val
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/val,
		/obj/item/weapon/ranged/bullet/magazine/rifle/val,
		/obj/item/magazine/rifle_939,
		/obj/item/magazine/rifle_939,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/val = 1,
		/obj/item/magazine/rifle_939 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/rifle/vintorez
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/vintorez,
		/obj/item/weapon/ranged/bullet/magazine/rifle/vintorez,
		/obj/item/magazine/rifle_939,
		/obj/item/magazine/rifle_939,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/vintorez = 1,
		/obj/item/magazine/rifle_939 = 4,
	)
	loot_count = 6
	chance_none = 25

/loot/weapon/explosive/grenade_launcher //Lavaland only.
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/revolver/grenade_launcher,
		/obj/item/weapon/ranged/bullet/revolver/grenade_launcher,
		/obj/item/bullet_cartridge/grenade_40mm,
		/obj/item/bullet_cartridge/grenade_40mm
	)

	loot_table = list(
		/obj/item/bullet_cartridge/grenade_40mm = 4,
	)
	loot_count = 6
	chance_none = 25


/loot/weapon/rifle/moar
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/moar,
		/obj/item/weapon/ranged/bullet/magazine/rifle/moar,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/moar = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25


/loot/weapon/rifle/sol_burst
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/sol_burst,
		/obj/item/weapon/ranged/bullet/magazine/rifle/sol_burst,
		/obj/item/magazine/rifle_556,
		/obj/item/magazine/rifle_556,
	)

	loot_table = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/sol_burst = 1,
		/obj/item/magazine/rifle_556 = 4,
	)
	loot_count = 6
	chance_none = 25



/loot/weapon/rifle/sniper_20
	loot_table_guaranteed = list(
		/obj/item/weapon/ranged/bullet/magazine/rifle/sniper_20,
		/obj/item/magazine/sniper_20mm,
		/obj/item/magazine/sniper_20mm,
	)

	loot_table = list(
		/obj/item/magazine/sniper_20mm = 1,
	)
	loot_count = 4
	chance_none = 50
