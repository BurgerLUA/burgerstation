...
/ai/bot/medical/proc/find_healing_target()
	...
	if(next_idle_voice <= world.time)
		switch(rand(1,6))
			if(1)
				play_sound('sound/voice/medbot/apple.ogg',get_turf(owner))
				owner.do_say("An apple a day keeps me away!")
			if(2)
				play_sound('sound/voice/medbot/catch.ogg',get_turf(owner))
				owner.do_say("There's always a catch, and I'm the best there is.")
			if(3)
				play_sound('sound/voice/medbot/delicious.ogg',get_turf(owner))
				owner.do_say("Delicious!")
			if(4)
				play_sound('sound/voice/medbot/flies.ogg',get_turf(owner))
				owner.do_say("What kind of medbay is this? Everyone is dropping like flies!")
			if(5)
				play_sound('sound/voice/medbot/radar.ogg',get_turf(owner))
				owner.do_say("Radar! Put a mask on!")
			if(6)
				play_sound('sound/voice/medbot/surgeon.ogg',get_turf(owner))
	...
	if(healing_target != best_target)
		...
		if(healing_target)
			switch(rand(1,3))
				if(1)
					play_sound('sound/voice/medbot/injured.ogg',get_turf(owner))
					owner.do_say("You appear to be injured, [healing_target.name]!")
				if(2)
					play_sound('sound/voice/medbot/coming.ogg',get_turf(owner))
					owner.do_say("Hold on [healing_target.name], I'm coming!")
				if(3)
					play_sound('sound/voice/medbot/help.ogg',get_turf(owner))
