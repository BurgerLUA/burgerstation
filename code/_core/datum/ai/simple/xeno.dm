...
/ai/xeno/handle_attacking()
	...
	if(!. && objective_attack && objective_distance <= VIEW_RANGE && is_living(objective_attack))
		if(X.can_leap && world.time >= next_leap && objective_distance >= 3)
			...
			owner.throw_self(owner,objective_attack,16,16,offsets[1]*throw_velocity,offsets[2]*throw_velocity,lifetime = SECONDS_TO_DECISECONDS(4), steps_allowed = 6, desired_loyalty_tag = owner.loyalty_tag)
