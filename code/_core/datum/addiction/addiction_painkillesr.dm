...
/addiction/painkillers/on_life(mob/living/advanced/A,obj/item/organ/internal/brain/B,addiction_value=0,withdrawal_value=0)
	...
	if(withdrawal_value >= 15 && prob(withdrawal_value))
		...
		switch(rand(1,3))
			...
			if(2)
				A.tox_regen_buffer -= TICKS2DS(LIFE_TICK_SLOW)
				A.to_chat(span("warning","You feel sick..."))
			if(3)
				A.burn_regen_buffer -= TICKS2DS(LIFE_TICK_SLOW)
