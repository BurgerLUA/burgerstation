...
/obj/item/container/food/dynamic/cheese/on_damage_received(var/atom/atom_damaged,var/atom/attacker,var/atom/weapon,var/list/damage_table,var/damage_amount,var/critical_hit_multiplier,var/stealthy=FALSE)
	...
	if(icon_state == "wheel")
		...
		if(is_living(attacker))
			var/mob/living/L = attacker
			L.visible_message(span("notice","\The [caller.name] cuts \the [src.name] into several cheese pieces."),span("notice","You cut \the [src.name] into [pieces] cheese pieces."))
		...
		if(original_volume > 10)
			...
			if(is_living(attacker))
				var/mob/living/L = attacker
				L.to_chat(span("notice","\The [caller.name] cuts \the [src.name] into several cheese pieces."),span("notice","You cut \the [src.name] into [pieces] cheese pieces."))
			...
			if(is_living(attacker))
				var/mob/living/L = attacker
				L.visible_message(span("notice","\The [caller.name] slices a thin slice of cheese off the top of \the [src.name]."),span("notice","You slice a thin slice of cheese off the top of \the [src.name]."))
