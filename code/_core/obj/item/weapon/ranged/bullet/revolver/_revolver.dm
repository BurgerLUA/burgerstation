...
/obj/item/weapon/ranged/bullet/revolver/shoot(var/mob/activator,var/atom/object,location,params,var/damage_multiplier=1,var/click_called=FALSE)
...
/obj/item/weapon/ranged/bullet/revolver/get_shoot_delay(var/mob/activator,var/atom/target,location,params)

	. = ..()

	if(!activator.client || !has_quickshot)
		return .
...
/obj/item/weapon/ranged/bullet/revolver/can_load_chamber(var/mob/activator,var/obj/item/bullet_cartridge/B)
	return FALSE
...
/obj/item/weapon/ranged/bullet/revolver/click_self(var/mob/activator,location,control,params)
	...
	if(open)
		eject_stored_bullets_spent(activator,T,TRUE)
		activator.to_chat(span("notice","You open \the [src]. It contains [get_ammo_count()] bullet\s."))
	else
		activator.to_chat(span("notice","You close \the [src]."))
...
/obj/item/weapon/ranged/bullet/revolver/handle_ammo(var/mob/activator,var/bullet_position=1)
	var/obj/item/bullet_cartridge/B = spend_stored_bullet(activator,current_chamber)
	rotate_cylinder(1)
	return B

/obj/item/weapon/ranged/bullet/revolver/can_gun_shoot(var/mob/activator,var/atom/object,location,params,var/check_time=TRUE,var/messages=TRUE)

	if(!can_shoot_while_open && open)
		if(messages) activator.to_chat(span("warning","Close \the [src.name] before firing!"))
		return FALSE

	return ..()

/obj/item/weapon/ranged/bullet/revolver/clicked_on_by_object(var/mob/activator as mob,var/atom/object,location,control,params)
	...
	if(open && is_inventory(object) && src && is_inventory(src.loc)) //The revolver is in an inventory, and you clicked on it with your empty hands.
		...
		if(last_value)
			var/obj/item/bullet_cartridge/B = eject_stored_bullet(activator,last_value,get_turf(src))
			if(B)
				activator?.to_chat(span("notice","You remove \the [B.name] from \the [src.name]."))
				I.add_object(B)
