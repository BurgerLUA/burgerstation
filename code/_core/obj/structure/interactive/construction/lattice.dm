...
/obj/structure/interactive/construction/lattice/clicked_on_by_object(var/mob/caller,var/atom/object,location,control,params)
	...
	if(is_item(object))
		...
		if(istype(I,/obj/item/material/rod/))
			...
			INTERACT_CHECK_OBJECT
			INTERACT_DELAY(10)
			PROGRESS_BAR(caller,src,SECONDS_TO_DECISECONDS(1),.proc/construct_grille,caller,object)
		...
		if(istype(I,/obj/item/material/sheet/))
			...
			INTERACT_CHECK_OBJECT
			INTERACT_DELAY(10)
			PROGRESS_BAR(caller,src,SECONDS_TO_DECISECONDS(1),.proc/construct_floor_plating,caller,object)
