...
/obj/structure/interactive/construction/lattice/clicked_on_by_object(var/mob/caller,var/atom/object,location,control,params)
	...
	if(is_item(defer_object))
		...
		if(istype(I,/obj/item/material/rod/))
			...
			INTERACT_CHECK_DEFER
			INTERACT_DELAY(10)
			PROGRESS_BAR(caller,src,SECONDS_TO_DECISECONDS(3),.proc/construct_grille,caller,defer_object)
			PROGRESS_BAR_CONDITIONS(caller,src,.proc/can_construct_grille,caller,defer_object)
			return TRUE
		if(istype(I,/obj/item/material/sheet/))
			INTERACT_CHECK
			INTERACT_CHECK_DEFER
			INTERACT_DELAY(10)
			PROGRESS_BAR(caller,src,SECONDS_TO_DECISECONDS(3),.proc/construct_floor_plating,caller,defer_object)
			PROGRESS_BAR_CONDITIONS(caller,src,.proc/can_construct_floor_plating,caller,defer_object)
			return TRUE
