...
/mob/living/vehicle/mech/modular/can_attach_weapon(var/mob/caller,var/obj/item/I)
	...
	if(!mech_arms)
		caller?.to_chat(span("notice","You must add a set of arms to this assembly before attaching \the [I.name]!"))
		return FALSE
	...
	if(is_item(object) && is_living(caller))
		...
		if(istype(A,/obj/item/mech_part/))
			...
			if(istype(I,/obj/item/mech_part/arms))
				if(!mech_arms)
					if(!mech_body)
						caller?.to_chat(span("notice","You must add arms to this assembly before attaching \the [I.name]!"))
						return TRUE
					mech_arms = I
					. = TRUE
			...
			if(istype(I,/obj/item/mech_part/body))
				if(!mech_body)
					if(!mech_legs)
						caller?.to_chat(span("notice","You must add legs to this assembly before attaching \the [I.name]!"))
						return TRUE
					mech_body = I
					. = TRUE
			if(istype(I,/obj/item/mech_part/head))
				if(!mech_head)
					if(!mech_body)
						caller?.to_chat(span("notice","You must add a body to this assembly before attaching \the [I.name]!"))
						return TRUE
					mech_head = I
					. = TRUE
			if(.)
