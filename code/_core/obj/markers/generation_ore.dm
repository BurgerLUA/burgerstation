...
/obj/marker/generation/ore/New(desired_loc)
	. = ..()
...
/obj/marker/generation/ore/floor/get_object_to_place(turf/T,objects_placed=0,grow_left=0)
...
/obj/marker/generation/ore/wall/New(atom/desired_loc)
	. = ..()
	if(desired_loc && is_wall(desired_loc))
		turf_whitelist = desired_loc.type

/obj/marker/generation/ore/wall/get_object_to_place(turf/T,objects_placed=0,grow_left=0)
	var/obj/structure/interactive/ore_deposit/OD = new object_to_place(T)
	OD.material_id = selected_ore_type
	OD.ore_amount = 5
	return OD
