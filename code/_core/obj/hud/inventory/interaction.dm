/obj/hud/inventory/click_on_object(mob/caller as mob,atom/object,location,control,params) //The src is used on the object
	...
	if(caller.attack_flags & CONTROL_MOD_THROW && is_living(caller))
		...
		if(istype(object_to_throw))
			...
			//Throwing it
			object_to_throw.throw_self((grabbed_object ? grabbed_object : caller),object,target_cords[1],target_cords[2],vel_x,vel_y,lifetime=1 SECONDS,desired_loyalty_tag = L.loyalty_tag)
