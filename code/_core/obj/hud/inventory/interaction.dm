/obj/hud/inventory/click_on_object(var/mob/activator as mob,var/atom/object,location,control,params) //The src is used on the object
	...
	if(is_living(activator))
		...
		if(grabbed_object && grabbed_object == object && is_living(object))
			reinforce_grab(activator)
			return TRUE

	 //Wielding an object.
	if(parent_inventory)
		var/atom/top_object = parent_inventory.get_top_object()
		if(object != top_object && top_object.click_on_object_alt(activator,object,location,control,params))
			return TRUE

	var/atom/top_object = src.get_top_object() //What is in our inventory.

	//Alt click a corpse with an empty hand.
	if(!top_object && activator.attack_flags & CONTROL_MOD_DISARM && is_advanced(object))
		var/mob/living/advanced/A = object
		if(A.dead)
			A.examine_body_inventory(activator)
			return TRUE

	//Grabbing with an empty hand.
	if(!top_object && activator.attack_flags & CONTROL_MOD_GRAB)
		if(is_item(object))
			...
			if(is_inventory(I.loc) && !I.is_container)
				toggle_wield(activator,object)
				return TRUE
		if(is_organ(src.loc) && is_turf(object.loc) && get_dist(activator,object) <= 1)
			var/obj/item/organ/O = src.loc
			if(ismob(object))
				O.attack(activator,object,params,damage_type_override=/damagetype/unarmed/fists/grab)
			else
				src.grab_object(activator,object,location,control,params)
			return TRUE

	//Snowflake code for plates.
	if(istype(object.loc,/obj/item/plate)) //Plate fuckery.
		return src.click_on_object(activator,object.loc,location,control,params)

	//Alt clicking a movable, rotatable with an empty hand.
	if(!top_object && activator.attack_flags & CONTROL_MOD_DISARM && ismovable(object))
		...
		if(!M.anchored && M.can_rotate)
			...
			activator.to_chat(span("notice","You rotate \the [M.name] [rotation == -90 ? "clockwise" : "counter-clockwise"]."))
			return TRUE //Needs to be here. At this level.

	//Throw the object if we are telling it to throw.
	if(activator.attack_flags & CONTROL_MOD_THROW && is_living(activator))
		if(!is_turf(activator.loc))
			return TRUE
		var/mob/living/L = activator
		activator.face_atom(object) //Changing dir
		...
		if(istype(object_to_throw))
			if(is_living(object_to_throw))
				if(!allow_hostile_action(L.loyalty_tag,object_to_throw))
					activator.to_chat(span("warning","You can't throw allies!"))
					return TRUE
				if(grab_level < 2) //To throw mob you need an agressive grab
					activator.to_chat(span("warning","You need a better grip to do that!"))
					return TRUE
			var/vel_x = object.x - activator.x //Activator pos is the same as object anyway
			var/vel_y = object.y - activator.y
			...
			if(is_item(object_to_throw)) //Item needs aditional procedures
				...
				if(I.additional_clothing_parent || I.no_drop) //Can't throw additional clothing
					activator.to_chat(span("warning","You can't throw this!"))
					return TRUE
				if(!highest) //We want to throw it in us?.. No velocity
					I.drop_item(get_turf(activator)) //Drop if we can't throw.
					return TRUE
				I.drop_item(get_turf(activator),silent=TRUE)

			if(highest == 0) //Our position is the same as target
				release_object(activator) //Better not to divide by zero
				return TRUE
			...
			if(grabbed_object == object_to_throw)
				grabbed_object.Move(get_turf(activator)) //Anti-offset
				release_object(activator)

			var/list/target_cords = L.get_current_target_cords(params)
			//Throwing it
			object_to_throw.throw_self((grabbed_object ? grabbed_object : activator),object,target_cords[1],target_cords[2],vel_x,vel_y,lifetime=SECONDS_TO_DECISECONDS(1),desired_loyalty_tag = L.loyalty_tag)

		else if(top_object)
			activator.to_chat(span("warning","You can't throw \the [top_object.name]!"))

		return TRUE

	//Drop the object if we are telling it to drop.
	if(activator.attack_flags & CONTROL_MOD_DROP)
		if(parent_inventory)
		...
		if(is_item(top_object))
			...
			if(I.no_drop)
				activator.to_chat(span("warning","You can't drop this!"))
				return TRUE
		if(grabbed_object)
			release_object(activator)
			return TRUE
		...
		if(desired_turf && (istype(object,/obj/structure/table) || istype(object,/obj/item/plate)) && get_dist(activator_turf,desired_turf) <= 1)
	...
	if(activator.attack_flags & CONTROL_MOD_OWNER && top_object)
		top_object.click_on_object(activator,activator,location,control,params)
		return TRUE

	//Self clicking.
	if(top_object && (object == top_object || activator.attack_flags & CONTROL_MOD_SELF)) //Click on ourself
		top_object.click_self(activator)
		return TRUE
	else if(activator.attack_flags & CONTROL_MOD_SELF) //Z click wielding.
		if(is_advanced(activator))
			...
			if(src == A.inventories_by_id[BODY_HAND_RIGHT_HELD] && left_item)
				A.inventories_by_id[BODY_HAND_RIGHT_HELD].toggle_wield(activator,left_item)
				return TRUE
			if(src == A.inventories_by_id[BODY_HAND_LEFT_HELD] && right_item)
				A.inventories_by_id[BODY_HAND_LEFT_HELD].toggle_wield(activator,right_item)
				return TRUE

	//Handle moving grabbed objects
	if(grabbed_object && is_turf(grabbed_object.loc))
		if(is_turf(object) && (get_dist(activator,object) <= 1 || get_dist(object,grabbed_object) <= 1))
			...
			if(is_living(grabbed_object) && is_living(activator))
	...
	if(object.plane >= PLANE_HUD || (object.loc && object.loc.plane >= PLANE_HUD) || is_turf(object) || is_turf(object.loc)) // Only interact with objects that can actually be clicked on.
		if(get_dist(src,object) <= 1)
			if(is_item(object)) //We're clicking on another item.
				var/obj/item/I = object
				if(I.is_container && (I.anchored || !is_turf(I)) && activator.attack_flags & CONTROL_MOD_GRAB) //We're clicking on a container and we want to quickly grab the first object.
					...
					if(is_inventory(I.loc)) //The object we're clicking on is in an inventory. Special behavior.
						...
						if(!top_object) //We're clicking on an object with an empty hand.
							if(INV.worn) //We're clicking on a worn object.
								...
								if(content_length > 1 && activator.attack_flags & CONTROL_MOD_DISARM) //Force
									content_length -= 1
								for(var/i=content_length,i>0,i--)
									var/obj/item/ITM = INV.contents[i]
									if(ITM.click_self(activator))
										return TRUE
							...
							return TRUE
					else if(!top_object && activator.attack_flags && !(activator.attack_flags & ~(CONTROL_MOD_LEFT|CONTROL_MOD_RIGHT)) ) //If we don't have a top object and we don't have any non-left attack flags, pick it up.
						src.add_object(object)
						return TRUE
	...
	if(top_object && top_object.click_on_object(activator,object,location,control,params))
		return TRUE

	if(object.clicked_on_by_object(activator,top_object ? top_object : src,location,control,params))
		return TRUE

	if(top_object)
		top_object.attack(activator,object,params)
		return TRUE

	if(is_organ(src.loc)) //We belong to an organ, so we should attack.
		var/obj/item/organ/O = src.loc
		O.attack(activator,object,params)
		return TRUE

	return TRUE //Returning TRUE here as returning false would just run the above.

/obj/hud/inventory/proc/toggle_wield(var/mob/activator,var/obj/item/item_to_wield)

	if(parent_inventory)
		unwield(activator,item_to_wield)
	else
		wield(activator,item_to_wield)

	return TRUE


/obj/hud/inventory/proc/can_wield(var/mob/activator,var/obj/item/item_to_wield)

	if(src.is_occupied())
		activator.to_chat(span("warning","Your hand must be unoccupied in order to wield this!"))
		return FALSE

	if(!is_inventory(item_to_wield.loc) || item_to_wield.loc == src)
		activator.to_chat(span("warning","You must be holding this object in another hand in order to wield this!"))
		return FALSE
...
/obj/hud/inventory/proc/unwield(var/mob/activator,var/obj/item/item_to_wield)

	activator.to_chat(span("notice","You unbrace \the [item_to_wield] with your [src.loc.name]."))
...
/obj/hud/inventory/proc/wield(var/mob/activator,var/obj/item/item_to_wield)

	if(!is_item(item_to_wield) || !item_to_wield.can_wield)
		activator.to_chat(span("warning","You can't wield this!"))
		return FALSE

	if(src.parent_inventory)
		activator.to_chat(span("warning","You are already wielding something!"))
		return FALSE

	if(!can_wield(activator,item_to_wield))
		return FALSE

	activator.to_chat(span("notice","You brace \the [item_to_wield] with your [src.loc.name]."))
...
/obj/hud/inventory/dropped_on_by_object(var/mob/activator,var/atom/object,location,control,params) //Object dropped on src
...
/obj/hud/inventory/defer_click_on_object(var/mob/activator,location,control,params)
...
/obj/hud/inventory/on_mouse_up(var/mob/activator as mob, var/atom/object,location,control,params)

	var/atom/top_object = get_top_object()

	if(top_object && top_object.on_mouse_up(activator,object,location,control,params))
		return TRUE
