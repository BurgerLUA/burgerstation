/mob/proc/add_button(obj/hud/button/B) //This should never actually be used to add buttons. Call update_owner(M) instead on the button.
	buttons += B
	if(client)
		client.screen += B
	sync_buttons()

/mob/proc/remove_button(obj/hud/button/B) //This should never actually be used to remove buttons. Call update_owner(null) instead on the button.
	buttons -= B
	if(client)
		client.screen -= B
	sync_buttons()
...
//Health
/mob/proc/toggle_health(show_flags_whitelist=FLAG_HUD_ALL,show_flags_blacklist=FLAG_HUD_NONE,speed = 1 SECONDS)
	draw_health = !draw_health
	show_health(draw_health,show_flags_whitelist,show_flags_blacklist,speed)

/mob/proc/show_health(show=TRUE,show_flags_whitelist,show_flags_blacklist,speed=1 SECONDS)
...
//Buttons
/mob/proc/toggle_buttons(show_flags_whitelist=FLAG_HUD_ALL,show_flags_blacklist=FLAG_HUD_NONE,speed = 1 SECONDS)
	draw_buttons = !draw_buttons
	show_buttons(draw_buttons,show_flags_whitelist,show_flags_blacklist,speed)

/mob/proc/show_buttons(show=TRUE,show_flags_whitelist,show_flags_blacklist,speed=1 SECONDS)
	for(var/k in buttons)
		var/obj/hud/button/B = k
...
//HUD
/mob/proc/show_hud(show,show_flags_whitelist=FLAG_HUD_ALL,show_flags_blacklist=FLAG_HUD_NONE,speed=1 SECONDS)
...
/mob/proc/display_turf_contents(turf/T)
