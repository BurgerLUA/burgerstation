...
/mob/living/proc/get_skill(var/id)
	if(!skills[id])
		log_error("Warning! Tried getting skill of [id], but it didn't exist for [src.get_debug_name()]!")
		return null
	return skills[id]
...
/mob/living/proc/get_attribute(var/id)
	if(!attributes[id])
		log_error("Warning! Tried getting attribute of [id], but it didn't exist for [src.get_debug_name()]!")
		return null
