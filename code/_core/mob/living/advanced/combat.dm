...
/mob/living/advanced/perform_block(var/atom/attacker,var/atom/weapon,var/atom/target,var/damagetype/DT)
	...
	if(A && istype(A))
		play_sound('sounds/weapons/generic_block.ogg', vector(src.x,src.y,src.z), environment = A.sound_environment, volume = Clamp(100 - base_chance,25,100), sound_type = SOUND_TYPE_FX )
