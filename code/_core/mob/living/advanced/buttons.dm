...
/mob/living/advanced/proc/add_chargen_buttons(list/blacklist=list())
	var/species/S = SPECIES(species)
...
//Inventory
/mob/living/advanced/proc/toggle_inventory(show_flags_whitelist=FLAG_HUD_ALL,show_flags_blacklist=FLAG_HUD_NONE,speed = 1 SECONDS)
	return show_inventory(!draw_inventory,show_flags_whitelist,show_flags_blacklist,speed)

/mob/living/advanced/proc/show_inventory(show=TRUE,show_flags_whitelist,show_flags_blacklist,speed=1 SECONDS)
	draw_inventory = show
...
/mob/living/advanced/show_hud(show,show_flags_whitelist=FLAG_HUD_ALL,show_flags_blacklist=FLAG_HUD_NONE,speed=1 SECONDS)
	. = ..()
