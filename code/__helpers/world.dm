...
#define INITIALIZE(D)																								\
	if(D.initialized) {																								\
		...
		if(!D.Initialize()) {CRASH("ERROR: [D.get_debug_name()] did not run Initialize() properly!")};			\
		if(!D.PostInitialize()) {CRASH("ERROR: [D.get_debug_name()] did not run PostInitialize() properly!")};	\
		D.initialized = TRUE;																						\
	}
...
#define GENERATE(D)																									\
	if(D.generated)	{																								\
		...
		if(!D.Generate()) {CRASH("ERROR: [D.get_debug_name()] did not run Generate() properly!")};				\
		D.generated = TRUE;																							\
	}
...
#define FINALIZE(D)																									\
	if(D.finalized) {																								\
		...
		if(!D.Finalize()) {CRASH("ERROR: [D.get_debug_name()] did not run Finalize() properly!")};				\
		D.finalized = TRUE;																							\
	}
