...
#define SAVEATOM(varname) if(vars[varname]) .[varname] = vars[varname].save_item_data(P,save_inventory,died,loadout=loadout)
#define LOADATOM(varname) if(object_data[varname]) vars[varname] = load_and_create(P,object_data[varname],src,loadout=loadout)
...
#define LOADCONTENTS if(length(object_data["contents"])) {for(var/k in object_data["contents"]) load_and_create(P,k,src,loadout=loadout)}