#define PLANE_HIDDEN -100

#define PLANE_SPACE -90

#define PLANE_PARALLAX -80

#define PLANE_FLOOR_BELOW -50 //For openspace.

#define PLANE_FLOOR -16

#define PLANE_SHUTTLE -15

#define PLANE_WALL -14

#define PLANE_DECAL -13

#define PLANE_BLOOD -12

#define PLANE_JUNK -11 //Plane used for junk objects that shouldn't have outlines.

#define PLANE_OBJ -10

#define PLANE_CURRENCY -9

#define PLANE_MOB_STEALTH -8

#define PLANE_MOB -7

#define PLANE_SCENERY -6

#define PLANE_EFFECT -5

#define PLANE_AREA -4

#define PLANE_LIGHTING -3

#define PLANE_PARALLAX_LIGHTING -2

#define PLANE_EFFECT_LIGHTING -1

#define PLANE_DARKNESS 0 // This is always 0. There is no way to change this.

#define PLANE_DAMAGE_NUMBERS 15

#define PLANE_AUGMENTED 16

#define PLANE_CHAT 17

#define PLANE_ALWAYS_VISIBLE 99

#define PLANE_HUD 100

#define PLANE_HUD_OBJ 101

#define PLANE_HUD_MOUSE 102
