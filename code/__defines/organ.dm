#define FLAG_ORGAN_NONE 0x0
#define FLAG_ORGAN_BEAST_FEET 0x1
#define FLAG_ORGAN_BEAST_HEAD 0x2
#define FLAG_ORGAN_BEAST_WINGS 0x4

#define BODY_HEAD "head"
#define BODY_FACE "face"
#define BODY_HEAD_MALE "head_m"
#define BODY_HEAD_FEMALE "head_f"
#define BODY_NECK "neck"

#define BODY_TORSO "torso"
#define BODY_TORSO_O "torso_o"
#define BODY_TORSO_B "torso_b"
#define BODY_TORSO_OB "torso_ob"
#define BODY_TORSO_MALE "torso_m"
#define BODY_TORSO_FEMALE "torso_f"

#define BODY_GROIN "groin"
#define BODY_GROIN_O "groin_o"
#define BODY_GROIN_MALE "groin_m"
#define BODY_GROIN_FEMALE "groin_f"

#define BODY_ARM_LEFT "arm_l"
#define BODY_ARM_RIGHT "arm_r"

#define BODY_HAND_LEFT "hand_l"
#define BODY_HAND_RIGHT "hand_r"

#define BODY_HAND_LEFT_HELD "hand_l_held"
#define BODY_HAND_RIGHT_HELD "hand_r_held"

#define BODY_LEG_LEFT "leg_l"
#define BODY_LEG_RIGHT "leg_r"

#define BODY_FOOT_LEFT "foot_l"
#define BODY_FOOT_RIGHT "foot_r"

#define BODY_EYES "eyes"
#define BODY_EYE_LEFT "eye_l"
#define BODY_EYE_RIGHT "eye_r"

#define BODY_EARS "ears"
#define BODY_EAR_LEFT "ear_l"
#define BODY_EAR_RIGHT "ear_r"

#define BODY_RING_01 "ring_01"
#define BODY_RING_02 "ring_02"
#define BODY_RING_03 "ring_03"
#define BODY_RING_04 "ring_04"

#define BODY_TAIL "tail"

#define BODY_WINGS "wings"

#define BODY_ANTENNAE "antennae"

#define BODY_HAIR_HEAD "hair_head"
#define BODY_HAIR_FACE "hair_face"

#define BODY_SPECIAL "special"


#define BODY_BRAIN "brain"
#define BODY_HEART "heart"
#define BODY_LUNGS "lung"
#define BODY_STOMACH "stomach"
#define BODY_LIVER "liver"
#define BODY_INTESTINTES "intestines"
#define BODY_KIDNEYS "kidneys"

#define LEFT_HAND 0x1
#define RIGHT_HAND 0x2

#define LEFT_FOOT 0x4
#define RIGHT_FOOT 0x8

#define BODY_MAIN list(BODY_HEAD,BODY_TORSO,BODY_ARM_LEFT,BODY_ARM_RIGHT,BODY_HAND_LEFT,BODY_HAND_RIGHT,BODY_GROIN,BODY_LEG_LEFT,BODY_LEG_RIGHT,BODY_FOOT_LEFT,BODY_FOOT_RIGHT,BODY_EYES,BODY_EARS)